"""
Tests for the page allowing users to upload a csv file containing pupil data.
"""
...
class TestUploadPupilData(TestClient):
    ...
    def test_valid_csv_file_gets_processed_to_db(
        ...
        url = UrlName.PUPIL_UPLOAD.url()
        page = self.client.get(url)

        # Check response ok
        assert page.status_code == 200

        # Upload some valid pupil data as a csv file
        csv_file = get_csv_from_lists(
            [
                ["pupil_id", "firstname", "surname", "year_group_id"],
                [1, "Test", "Testson", yg.year_group_id],
            ]
        )
        upload_file = Upload(filename="pupils.csv", content=csv_file.read())
        ...
        pupil = pupils.get()
        assert pupil.pupil_id == 1
        assert pupil.firstname == "Test"
        assert pupil.surname == "Testson"
    ...
    def test_invalid_upload_rejected(self, mock_error_messages: mock.Mock()):
        ...
        # Create some invalid pupil data (non-unique pupil id)
        form = page.forms["upload-form"]
        csv_file = get_csv_from_lists(
            [
                ["pupil_id", "firstname", "surname", "year_group_id"],
    ...
    def test_can_download_example_file_then_upload_it(self):
        ...
        file_response = page.click(href=UrlName.PUPIL_DOWNLOAD.url())
        ...
        # Re-upload the file
        file = Upload(filename="pupils.csv", content=file_response.body)
        form = page.forms["upload-form"]
        form["csv_file"] = file
        upload_response = form.submit()

        # Check upload successful
        assert models.Pupil.objects.count() > 0
        assert upload_response.location == UrlName.PUPIL_LIST.url()
