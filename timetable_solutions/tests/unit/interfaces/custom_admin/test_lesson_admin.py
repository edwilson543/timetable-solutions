...
class TestLessonModelAdmin(test.TestCase):
    ...
    def test_save_model_from_valid_change_form(self):
        ...
        lesson = models.Lesson.objects.get_individual_lesson(
            school_id=123456, lesson_id=lesson_id
        )
        self.assertEqual(
            lesson.school, models.School.objects.get_individual_school(school_id=123456)
        )

        expected_pupils = models.Pupil.objects.get_all_instances_for_school(
            school_id=123456
        ).filter(pupil_id__in=[1, 2])
        ...
        expected_slots = models.TimetableSlot.objects.get_specific_timeslots(
            school_id=123456, slot_ids=[1, 2]
        )
        self.assertQuerysetEqual(
            lesson.user_defined_time_slots.all(), expected_slots, ordered=False
        )
    ...
    def test_get_queryset_lessons_filters_by_school(self):
        ...
        lesson_admin = admin.LessonAdmin(
            model=models.Lesson, admin_site=admin.user_admin
        )
        ...
        # Check outcome
        expected_queryset = models.Lesson.objects.get_all_instances_for_school(
            school_id=123456
        )
