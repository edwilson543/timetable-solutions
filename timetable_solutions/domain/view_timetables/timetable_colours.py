...
class TimetableColourAssigner:
    ...
    _regex_dict: dict[Pattern, Colour] = {
        ...
        re.compile(
            "(.*break.*)|(.*recreation.*)|(.*play.*time.*)", flags=re.IGNORECASE
        ): Colour.BREAK,
        re.compile(
            "(.*free.*)|(.*empty.*)|(.*spare.*)", flags=re.IGNORECASE
        ): Colour.FREE,
    }
    ...
    @classmethod
    def get_colours_for_pupil_timetable(
        cls, lessons: models.LessonQuerySet
    ) -> dict[str, str]:
        """
        ...
        uncleaned_lesson_counts = {
            lesson.subject_name: lesson.total_required_slots for lesson in lessons
        }
        lesson_counts = {
            subject_name: count
            for subject_name, count in uncleaned_lesson_counts.items()
            if subject_name not in generic_period_colours.keys()
        }

        # Rank the lessons ensuring all final ranks are in the specified colour ranking
        counts_ser = pd.Series(
            lesson_counts
        )  # Series used to access the .rank() method
        rank_ser = (
            counts_ser.rank(method="first", ascending=False) - 1
        )  # -1 so that ranks start at 0
        ...
        ranked_colours = {
            subject_name: cls._colour_ranking[rank]
            for subject_name, rank in lesson_rank_dict.items()
        }
        lesson_colours = {**generic_period_colours, **ranked_colours}
        return lesson_colours

    @classmethod
    def get_colours_for_teacher_timetable(
        cls, lessons: models.LessonQuerySet
    ) -> dict[str | int, str]:
        """
    ...
    # HELPER METHODS
    @classmethod
    def _get_generic_period_colours(
        cls, lessons: models.LessonQuerySet
    ) -> dict[str, str]:
        """
        ...
        generic_period_colours = {
            lesson.subject_name: matched_colour_code.value
